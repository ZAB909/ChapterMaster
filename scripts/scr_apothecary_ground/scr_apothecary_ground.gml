/*function scr_apothecary_ground() {
	...
	var p=0,apoth=0,company=0,v=0,tick=0,repair=0,engineers=[],unit;
	repair=50;
	    ...
	    if (p_player[p]>0){
	        ...
	        for(company=0;company<11;company++){
	           for (v=0;v<500;v++){
	           		...
	           		if (unit.is_at_location(name,p,0)){
		            ...
		            if (v<array_length(obj_ini.veh_race[company])){
		            	if (obj_ini.veh_loc[company][v]==name && obj_ini.veh_wid[company][v]==p){
