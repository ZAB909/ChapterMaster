#macro DEFAULT_VIEW_OFFSET 36
#macro DEFAULT_TOOLTIP_X_OFFSET 24
#macro DEFAULT_LINE_GAP 24
#macro DEFAULT_TOOLTIP_WIDTH 300

function tooltip_draw(tooltip, rect_x=mouse_x+DEFAULT_TOOLTIP_X_OFFSET, rect_y=mouse_y, extra_x=0, extra_y=0, max_width=DEFAULT_TOOLTIP_WIDTH, line_gap=DEFAULT_LINE_GAP, draw_color=c_gray){
	...
	rect_x = clamp(rect_x, xx, xx + __view_get(e__VW.WView, 0) - width - DEFAULT_VIEW_OFFSET);
	rect_y = clamp(rect_y, yy, yy + __view_get(e__VW.HView, 0) - height - DEFAULT_VIEW_OFFSET);
	draw_rectangle_with_color_and_alpha(rect_x, rect_y, width + rect_x + 6, height + rect_y + 6, 0, 0, 1)
	draw_rectangle_with_color_and_alpha(rect_x, rect_y, width + rect_x + 6, height + rect_y + 6, 1, draw_color, 1)
	draw_rectangle_with_color_and_alpha(rect_x + 1, rect_y + 1, width + rect_x + 5, height + rect_y + 5, 1, draw_color, 0.5)
	draw_text_ext_with_color_and_alpha(rect_x + 2.5, rect_y + 2.5, tooltip, line_gap, width, draw_color, 1)
	return [width + 6, height + 6]
}

function draw_rectangle_with_color_and_alpha(x, y, width, height, outline, color, alpha){
	draw_set_color(color);
	draw_set_alpha(alpha);
	draw_rectangle(x, y, width, height, outline);
}

function draw_text_ext_with_color_and_alpha(x, y, string, sep, w, color, alpha){
	draw_set_color(color);
	draw_set_alpha(alpha);
