...
function scr_powers(power_set, power_count, enemy_target, unit_id) {
	...
	var school,power_name,target_type,enemy5,onc,using,binders,book_powers,book_roll,tome_bad,tome_slot,tome_tags,damnyou;
	var school="";
	var power_name="";
	var target_type="";
	enemy5=enemy_target;
	    ...
	    if (unit.hp()<0){
	        ...
	        repeat(30){// Need to find the open slot
	            ...
	            if (obj_ncombat.player_forces>6){
	                if (marine_type[unit_id]=lost[hh]) and (good=0){lost_num[hh]+=1;good=1;}// If one unit is all the same, and get smashed, this should speed up the repeats
	                if (marine_type[unit_id]=lost[h]) and (good=0){lost_num[h]+=1;good=1;hh=h;}
	            }
	...
	if (p_type="buff") or (power_name="gather_energy"){
	    ...
	    if (power_name="Fire Shield"){
	        ...
	        repeat(100){
	            ...
	            if (buf=0){if (marine_fshield[unit_id]<2){buf-=1;marine_fshield[unit_id]=2;}}
	        }
	    }
