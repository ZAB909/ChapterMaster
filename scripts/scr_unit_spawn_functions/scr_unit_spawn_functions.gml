...
function scr_marine_trait_spawning(distribution_set){
	...
	for (var i=0;i<array_length(distribution_set);i++){//standard distribution for trait
		if (array_length(distribution_set[i])==2){
			...
			if (struct_exists(other_spawn_data, "recruit_data")){
				var recruit_world_data = [other_spawn_data, "recruit_data"];
...
function scr_compile_trial_bonus_string(trial_data){
	...
	if (struct_exists(trial_data,"train_time")){
		...
		bonus_string+="tyears training : " + train_time_string(train_time_data.base);
