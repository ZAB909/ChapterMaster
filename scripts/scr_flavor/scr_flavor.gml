function scr_flavor(number_of_attacking_weapons, target, target_type, number_of_shots, casulties) {
	...
	if (string_count("Bolt",wepp)>0) and (solod=false){flavored=1;
	    ...
	    if (number_of_shots<200){
	        ...
	        if (target.dudes_num[targeh]>1) and (casulties=0) then p1=$"{number_of_shots} {wepp} fire strike the enemy ranks.";
	        if (target.dudes_num[targeh]>1) and (casulties>0) then p1=$"{number_of_shots} {wepp} rip through the enemy ranks.";
	    }
	    if (number_of_shots>=200){
	        ...
	        if (target.dudes_num[targeh]>1) and (casulties=0) then p1=$"{number_of_shots} {wepp}s rip through the enemy ranks.";
	        if (target.dudes_num[targeh]>1) and (casulties>0) then p1=$"{number_of_shots} {wepp}s rip through the enemy ranks.";
	    }
	}
	if (string_count("Bolt",wepp)>0) and (solod=true){flavored=1;
	    if (target.dudes_num[targeh]>1) and (casulties=0) then p1=string(full_name)+" fires his {wepp} into the {target_name} ranks.";
	    if (target.dudes_num[targeh]>1) and (casulties>0) then p1=string(full_name)+" fires his {wepp} into the {target_name} ranks.";
	...
	if (wepp="hammer_of_wrath"){
	    if (solod=false){flavored=1;
	        ...
	        if (target.dudes_num[targeh]>1) and (casulties=0) then p1+="they smash into the {target_name} ranks.";
	        if (target.dudes_num[targeh]>1) and (casulties>0) then p1+="they smash into the {target_name} ranks.";
	    ...
	    if (solod=true){
	        ...
	        if (target.dudes_num[targeh]>1) then p1=string(full_name)+" activates his Jump Pack and launches up into the air, and then crashes down into the {target_name} ranks- ";
	...
	if (string_count("Plasma Pistol",wepp)>0) and (solod=false){flavored=1;
	    if (target.dudes_num[targeh]=1) then p1=$"{number_of_shots} {wepp} shoot bolts of energy into a {target_name}.";
	    if (target.dudes_num[targeh]>1) and (casulties=0) then p1=$"{number_of_shots} {wepp} shoot bolts of energy into the {target_name} ranks.";
	}
	...
	if (string_count("Flamer",wepp)>0) and (solod=false){flavored=1;
	    ...
	    if (target.dudes_num[targeh]>1) and (casulties>0) then p1=$"{number_of_shots} {wepp} bathe the {target_name} ranks in holy promethium.";
	}
	if (wepp="Missile Launcher") and (solod=false){flavored=1;
	    ...
	    if (target.dudes_num[targeh]>1) then p1=$"{number_of_shots} {wepp} fire on the {target_name} ranks.";
	}
	if (wepp="Whirlwind Missiles") and (solod=false){flavored=1;
	    ...
	    if (target.dudes_num[targeh]>1) then p1="{number_of_shots} Whirlwinds fire on the {target_name} ranks.";
	}
	...
	if ((wepp="Lascannon") or (wepp="Twin Linked Lascannon") or (wepp="Lascannons")) and (solod=false){flavored=1;
	    ...
	    if (number_of_shots>1) then p1=$"{number_of_shots} {wepp} fire upon the {target_name} ranks.";        
	}
	if (wepp="Webber") and (solod=false){flavored=1;
	    ...
	    if (target.dudes_num[targeh]>1) then p1=$"{number_of_shots} {wepp} spray ooze on the {target_name} ranks.";
	}
	...
	if (wepp="Close Combat Weapon") and (solod=false){flavored=1;
	    ...
	    if (number_of_shots>1) then p1=$"{number_of_shots} {obj_ini.role[100][6]}s stomp, wrench, and smash at the {target_name} ranks.";
	}
	if (wepp="Chainsword") and (solod=false){flavored=1;
	    if (number_of_shots=1) then p1=$"A {target_name} is struck by a {wepp}.";
	    if (number_of_shots>1) then p1="{number_of_shots} Chainsword motors rev and hack at the {target_name} ranks.";
	}
	if (wepp="Sarissa") and (solod=false){flavored=1;
	    if (number_of_shots=1) then p1=$"A {target_name} is struck by a Battle Sister's {wepp}.";
	    // if (number_of_shots>1) and (casulties=0) then p1=$"A Battle Sister "+choose("howls out","roars")+" and hacks at the {target_name} ranks with her Sarissa.";
	    if (number_of_shots>1) then p1="{number_of_shots} Battle Sisters "+choose("howl out","roar")+" as they hack away at the {target_name} ranks with their Sarissas.";
	}
	if (wepp="Eviscerator") and (solod=false){flavored=1;
	    if (number_of_shots=1) then p1=$"A {target_name} is struck by a {wepp}.";
	    if (number_of_shots>1) then p1="{number_of_shots} Eviscerators rev and howl, hacking at the {target_name} ranks.";
	}
	...
	if ((string_count("Power",wepp)>0) or (wepp="Lightning Claw")) and (solod=false){flavored=1;
	    ...
	    if (target.dudes_num[targeh]>1){
	        if (number_of_shots>1) then p1=$"{number_of_shots} {wepp}s crackle and spark, striking at the {target_name} ranks.";
	    }
	}
	...
	if (string_count("Bolt",wepp)>0) and (solod=false){flavored=1;
	    ...
	    if (number_of_shots<200){
	        ...
	        if (target.dudes_num[targeh]>1) and (casulties=0) then p1=$"{number_of_shots} {wepp} fire strikes the ranks of the {target_name} but seemingly does no damage.";
	    ...
	    if (number_of_shots>=200){
	        ...
	        if (target.dudes_num[targeh]>1) and (casulties=0) then p1=$"{number_of_shots} {wepp}s rip through the ranks of {target_name}, seemingly doing no damage.";
	        if (target.dudes_num[targeh]>1) and (casulties>0) then p1=$"{number_of_shots} {wepp}s rip through the ranks of {target_name}, {casulties} dying immediately.";
	    }
	}
	if (string_count("Bolt",wepp)>0) and (solod=true){flavored=1;
	    if (target.dudes_num[targeh]>1) and (casulties=0) then p1=string(full_name)+" fires his {wepp} into the {target_name} ranks.";
	    if (target.dudes_num[targeh]>1) and (casulties>0) then p1=string(full_name)+" fires his {wepp} into the {target_name} ranks and kills {casulties}.";
	    if (target.dudes_num[targeh]=1) and (casulties=0) then p1=string(full_name)+" fires his {wepp} into a {target_name} but fails to kill it.";
	    if (target.dudes_num[targeh]=1) and (casulties>0) then p1=string(full_name)+" fires his {wepp} into a {target_name}, killing it.";
	}
	...
	if (wepp="hammer_of_wrath"){
	    if (solod=false){flavored=1;
	        ...
	        if (target.dudes_num[targeh]>1) and (casulties=0) then p1+=" they smash into the {target_name} ranks but fail to kill any.";
	        if (target.dudes_num[targeh]>1) and (casulties>0) then p1+=" they smash into the {target_name} ranks and pulverize {casulties}.";
	        if (target.dudes_num[targeh]=1) and (casulties=0) then p1+="a {target_name} survives the attack.";
	...
	if (string_count("Plasma Pistol",wepp)>0) and (solod=false){flavored=1;
	    if (target.dudes_num[targeh]=1) and (casulties=0) then p1=$"{number_of_shots} {wepp} shoot bolts of energy into a {target_name}.";
	    if (target.dudes_num[targeh]=1) and (casulties=1) then p1=$"{number_of_shots} {wepp} overwhelm a {target_name} with bolts of energy.";
	    if (target.dudes_num[targeh]>1) and (casulties=0) then p1=$"{number_of_shots} {wepp} shoot bolts of energy into the {target_name} ranks.";
	...
	if (wepp="Assault Cannon") and (solod=false){flavored=1;
	    if (target.dudes_num[targeh]=1) and (casulties=0) then p1=$"{number_of_shots} {wepp}s hum and rotate, belching out bullets and flame alike.  Explosions clap across the armour of the {target_name}.";
	    if (target.dudes_num[targeh]=1) and (casulties=1) then p1=$"{number_of_shots} {wepp}s hum and rotate, belching out bullets and flame alike. A {target_name} is ripped apart by the rounds.";
	    if (target.dudes_num[targeh]>1) and (casulties=0) then p1=$"{number_of_shots} {wepp}s hum and rotate, belching out bullets and flame alike .  The {target_name}s are rocked but unharmed.";
	    if (target.dudes_num[targeh]>1) and (casulties>0) then p1=$"{number_of_shots} {wepp}s hum and rotate, belching out bullets and flame alike.  {casulties} {target_name} are purged.";
	}
	if (string_count("Flamer",wepp)>0) and (solod=false){flavored=1;
	    if (target.dudes_num[targeh]=1) and (casulties=0) then p1=$"{number_of_shots} {wepp} bathe the {target_name} in holy promethium.";
	    if (target.dudes_num[targeh]=1) and (casulties=1) then p1=$"{number_of_shots} {wepp} flash-fry the {target_name} inside its armour.";
	    if (target.dudes_num[targeh]>1) and (casulties=0) then p1=$"{number_of_shots} {wepp} wash over the {target_name} ranks.";
	    if (target.dudes_num[targeh]>1) and (casulties>0) then p1=$"{number_of_shots} {wepp} bathe the {target_name} ranks in holy promethium, cleansing {casulties}.";
	}
	if (wepp="Missile Launcher") and (solod=false){flavored=1;
	    ...
	    if (target.dudes_num[targeh]=1) and (casulties=1) then p1=$"{number_of_shots} {wepp} fire upon the {target_name} and blow it to small pieces.";
	    if (target.dudes_num[targeh]>1) and (casulties=0) then p1=$"{number_of_shots} {wepp} fire on the {target_name} ranks.";
	    if (target.dudes_num[targeh]>1) and (casulties>0) then p1=$"{number_of_shots} {wepp} fire upon the {target_name} ranks and blow {casulties} to pieces.";
	}
	if (wepp="Whirlwind Missiles") and (solod=false){flavored=1;
	    ...
	    if (target.dudes_num[targeh]>1) and (casulties=0) then p1="{number_of_shots} Whirlwinds fire on the {target_name} ranks.";
	    if (target.dudes_num[targeh]>1) and (casulties>0) then p1="{number_of_shots} Whirlwinds fire upon the {target_name} ranks and blow {casulties} to pieces.";
	}
	if (wepp="Sniper Rifle") and (solod=false){flavored=1;
	    if (target.dudes_num[targeh]=1) and (casulties=0) then p1=$"{number_of_shots} {wepp} fire upon the {target_name} and fail to do damage.";
	    if (target.dudes_num[targeh]=1) and (casulties=1) then p1=$"{number_of_shots} {wepp} fire upon the {target_name} and destroy it.";
	    if (target.dudes_num[targeh]>1) and (casulties=0) then p1=$"{number_of_shots} {wepp} fire on the {target_name} ranks.";
	    if (target.dudes_num[targeh]>1) and (casulties>0) then p1=$"{number_of_shots} {wepp} fire upon the {target_name} ranks and kill {casulties}.";
	}
	if (wepp="Stalker Pattern Bolter") and (solod=false){flavored=1;
	    if (target.dudes_num[targeh]=1) and (casulties=0) then p1=$"{number_of_shots} {wepp} fire upon the {target_name} and fail to do damage.";
	    if (target.dudes_num[targeh]=1) and (casulties=1) then p1=$"{number_of_shots} {wepp} fire upon the {target_name} and destroy it.";
	...
	if ((wepp="Lascannon") or (wepp="Twin Linked Lascannon") or (wepp="Lascannons")) and (solod=false){flavored=1;
	    if (wepp="Lascannons"){wepp="Lascannon";number_of_shots=number_of_shots*2;}
	    if (number_of_shots=1) and (casulties=0) then p1=$"A {target_name} is struck by a {wepp}.";
	    if (number_of_shots=1) and (casulties=1) then p1=$"A {target_name} is struck by a {wepp} and killed instantly.";
	    if (number_of_shots>1) and (casulties=0) then p1=$"{number_of_shots} {wepp} fire upon a {target_name} and fail to kill it.";
	    if (number_of_shots>1) and (casulties=1) then p1=$"{number_of_shots} {wepp} fire upon a {target_name} and turn it into paste.";
	...
	if (wepp="Webber") and (solod=false){flavored=1;
	    ...
	    if (target.dudes_num[targeh]>1) and (casulties=0) then p1=$"{number_of_shots} {wepp} spray ooze on the {target_name} ranks.";
	    if (target.dudes_num[targeh]>1) and (casulties>0) then p1=$"{number_of_shots} {wepp} spray ooze on the {target_name} ranks and immobilize {casulties} of them.";
	}
	...
	if (wepp="Chainsword") and (solod=false){flavored=1;
	    ...
	    if (number_of_shots>1) and (casulties=0) then p1="{number_of_shots} Chainsword motors rev and hack at the {target_name} ranks.";
	    if (number_of_shots>1) and (casulties>0) then p1="{number_of_shots} Chainsword motors rev and hack away at the {target_name} ranks.  {casulties} are cut down.";
	}
	if (wepp="Sarissa") and (solod=false){flavored=1;
	    ...
	    if (number_of_shots>1) and (casulties=0) then p1=$"A Battle Sister "+choose("howls out","roars")+" and hacks at the {target_name} ranks with her Sarissa.";
	    if (number_of_shots>1) and (casulties>0) then p1="{number_of_shots} Battle Sisters "+choose("howl out","roar")+" as they hack away at the {target_name} ranks with their Sarissas.  {casulties} are cut down.";
	}
	if (wepp="Eviscerator") and (solod=false){flavored=1;
	    if (number_of_shots=1) and (casulties=0) then p1=$"A {target_name} is struck by a {wepp} but survives.";
	    if (number_of_shots=1) and (casulties=1) then p1=$"A {target_name} is cut down by a {wepp}.";
	    if (number_of_shots>1) and (casulties=0) then p1="{number_of_shots} Eviscerators rev and howl, hacking at the {target_name} ranks.";
	    if (number_of_shots>1) and (casulties>0) then p1="{number_of_shots} Eviscerators rev and howl, hacking at the {target_name} ranks.  {casulties} are cut down.";
	}
	if (wepp="Force Staff") and (solod=false){flavored=1;
	...
	if ((string_count("Power",wepp)>0) or (wepp="Lightning Claw")) and (solod=false){flavored=1;
	    ...
	    if (target.dudes_num[targeh]>1){
	        if (number_of_shots>1) and (casulties=0) then p1=$"{number_of_shots} {wepp}s crackle and spark, striking at the {target_name} ranks.";
	        if (number_of_shots>1) and (casulties>0) then p1=$"{number_of_shots} {wepp}s crackle and spark, hewing through the {target_name} ranks.  {casulties} are cut down.";
	    }
	}
	if ((string_count("Power",wepp)>0) or (wepp="Force Staff") or (wepp="Inactive Force Staff") or (wepp="Lightning Claw")) and (solod=true){flavored=1;
	...
	if (flavored=0) and (solod=false){p1=$"{number_of_shots} {wepp}+|{target_name}|+{casulties}+|";}
