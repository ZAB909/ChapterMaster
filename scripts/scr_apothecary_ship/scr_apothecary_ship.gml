	        ...
	        if (maybe=1){
	            ...
	            if (maybe=2){
	                        if (unit.armour()!="Dreadnought") and (ship_apoth>0) and (unit.hp()<unit.max_health()) and (normal_hp=true){
								if (unit.base_group != "marine"){
									if (obj_ini.ossmodula=1){
										unit.update_health(unit.hp()+(unit.max_health()/8))
									}else{
										unit.update_health(unit.hp()+(unit.max_health()/4))
									}
								} else {
									unit.update_health(unit.hp()+(unit.max_health()/10))
								}
								ship_apoth--;                           
	                        }
