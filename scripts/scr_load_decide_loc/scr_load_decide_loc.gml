	    ...
	    if (role_type="command"){
	        for(i=1;i<101;i++){
	            ...
	            if (man[i]="man") and (ma_god[i]<10){
	                if (ma_role[i]=obj_ini.role[100][5]) or (ma_role[i]=obj_ini.role[100][14]) or (ma_role[i]=obj_ini.role[100][15]) or (ma_role[i]=obj_ini.role[100][11]) or (ma_role[i]="Company Champion") or (ma_role[i]="Champion"){
	                    j=0;good=0;
	                    for(j=1;j<91;j++){
	                    	if (good=0){
	                        if (location_list[j]=unit_location) and (planet_number_list[j]=ma_wid[i]){
