...
// https://manual.yoyogames.com/GameMaker_Language/GML_Reference/File_Handling/Encoding_And_Hashing/json_encode.htm
	...
	if (save_process_stage = 1) or(save_process_stage = 0) {
		...
		//ERROR add Save() functions, which will prepare structs, ready for saving
	}
}
...
function scr_save(save_process_stage, save_id, save_file) {
    ...
    if (save_process_stage = 1) or(save_process_stage = 0) {
        ...
        repeat(6) {
            j += 1;
            ini_write_real("Controller", "stc_bonus_" + string(j), obj_controller.stc_bonus[j]);
        }
    ...
    if (save_process_stage = 2) or(save_process_stage = 0) {
        ...
        num = instance_number(obj_temp_arti);
        instance_array = 0;
        for (i = 0; i < num; i += 1) {
        ...
        repeat(21) {
            i += 1;
            ini_write_string("Ini", "comp_title" + string(i), obj_ini.company_title[i]);
        }
        var i;
        i = -1;
        repeat(121) {
            i += 1;
            ini_write_real("Ini", "slave_num_" + string(i), obj_ini.slave_batch_num[i]);
            ini_write_real("Ini", "slave_eta_" + string(i), obj_ini.slave_batch_eta[i]);
        }
    ...
    if (save_process_stage = 3) or(save_process_stage = 0) {
        ...
        repeat(11) {
            i += 1;
            ini_write_string("Res", "tit" + string(i), obj_controller.restart_title[i]);
        }
        ...
        repeat(5) {
            ad += 1;
            ini_write_string("Res", "adv" + string(ad), obj_controller.restart_adv[ad]);
            ini_write_string("Res", "dis" + string(ad), obj_controller.restart_dis[ad]);
        }
