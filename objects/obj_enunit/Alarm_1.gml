...
//Leadership
	if (dudes[j]="Techpriest"){
		...
		scr_en_weapon("Conversion Beam Projector",true,dudes_num[j],dudes[j],j);
		scr_en_weapon("Flamer",true,dudes_num[j],dudes[j],j);
		dudes_ac[j]=0;dudes_hp[j]=40;men+=dudes_num[j];}// May be a little too weak for a squad/expedition leader and a hireling.
//Minor leaders
    if (dudes[j]="Thallax"){
	...
	if (dudes[j]="Skitarii"){
		...
		scr_en_weapon("Combat Knife",true,dudes_num[j],dudes[j],j);
		dudes_ac[j]=5;dudes_hp[j]=40;men+=dudes_num[j];}
