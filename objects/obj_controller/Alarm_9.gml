...
with(obj_p_fleet){
    ...
    for(var b=1; b<=4; b++){
        if (steh.p_first[b]<=5) and (steh.dispo[b]>-30) and (steh.dispo[b]<0) {
            steh.dispo[b]=min(obj_ini.imperium_disposition,obj_controller.disposition[2])+choose(-1,-2,-3,-4,0,1,2,3,4);
        }
    }
	if (steh.visited==0){
		for(var planet_num =1;planet_num<5;planet_num++){
		    if (array_length(steh.p_feature[planet_num])!=0) with(steh){
