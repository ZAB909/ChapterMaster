...
/*if (defeat=1) and (dropping=0){
    ...
    repeat(3300){
        ...
        if (cmp<11) and (i>0) and (i<=300){// Ran by obj_fleet to calculate the number of dead marines
            if (obj_ini.name[cmp,i]!="") and (obj_ini.wid[cmp,i]=battle_id) and (obj_ini.loc[cmp,i]=battle_loc) and (obj_ini.lid[cmp,i]=0){
                final_deaths+=1;clean[cmp]=1;
                
                if (obj_ini.role[cmp,i]="Chapter Master") then obj_controller.alarm[7]=1;
