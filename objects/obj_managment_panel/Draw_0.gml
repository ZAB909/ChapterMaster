slate_panel.inside_method = function(){
    ...
    if (header=3){
        ...
        if (line[1]!=""){
            ...
            draw_text_bold(x2+(wid/2),y2+102,string_hash_to_newline(line[1]));
        ...
        if (line[1]!=""){
            if (italic[1]=1) then draw_set_font(fnt_40k_12i);
            draw_text_bold(x2+(wid/2),y2+43,string_hash_to_newline(line[1]));
            draw_set_font(fnt_40k_12);
        }
        var l=1;
        repeat(10){l+=1;
...
if (header=3){
    ...
    wid=175;
    hei=200;
}else if (header=1){wid=150;hei=290;}
var x_scale = (wid/850)
var y_scale = (hei/860)
